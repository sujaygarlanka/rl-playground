...
def train_q_agent():
     ...
     for episode in range(num_episodes):
          ...
          for step in range(max_steps_per_episode):
               ...
               # print(agent.get_q_value(state, action))
               # print(next_state)
               # print(agent.get_q_value(next_state))
               # print(reward)
               new_q_value = lr * ((agent.get_q_value(next_state) * gamma) + reward) + (1 - lr) * agent.get_q_value(state, action)
               # print(new_q_value)
               agent.set_q_value(state, action, new_q_value)
...
def train_dqn_agent():
     ...
     for episode in range(num_episodes):
          ...
          score = 0
          for step in range(max_steps_per_episode):
               action = agent.select_action(np.asarray([state]), policy_net)
               reward, next_state, done = env.step(action)
               score += reward
